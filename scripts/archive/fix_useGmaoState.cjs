...
`;
content = content.replace(/import { safeNum } from '\.\.\/utils\/formulaEngine';/, "import { safeNum } from '../utils/formulaEngine';\n" + importsToAdd);

// Add the useEffect before return
const syncEffect = `
  useEffect(() => {
...
content = content.replace(/  return \{/g, syncEffect + "\n  return {");

fs.writeFileSync(filePath, content, 'utf8');
console.log("useGmaoState modified.");
