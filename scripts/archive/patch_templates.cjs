const fs = require('fs');
let content = fs.readFileSync('src/components/TemplatesView.jsx', 'utf8');

// 1. Imports
content = content.replace(
  "FolderTree, Cpu } from 'lucide-react';",
  "FolderTree, Cpu, Trash2, Edit2, AlertTriangle } from 'lucide-react';"
);
...
// 5. Row
content = content.replace(
  /<ArrowRight className="w-3 h-3 text-amber-600 group-hover:translate-x-0.5 transition-transform" \/>\s*<\/button>\s*<\/td>\s*<\/tr>/g,
...
fs.writeFileSync('src/components/TemplatesView.jsx', content);
console.log('TemplatesView.jsx patched');
