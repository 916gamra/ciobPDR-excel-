const fs = require('fs');
let content = fs.readFileSync('src/components/TypeView.jsx', 'utf8');

// 1. Imports
content = content.replace(
  "Package, Layers } from 'lucide-react';",
  "Package, Layers, Trash2, Edit2, AlertTriangle } from 'lucide-react';"
);
...
// 4. Header
content = content.replace(
  /<th className="py-2.5 px-4">\s*<span>NB ARTICLES<\/span> <span className="text-slate-400 font-normal text-\[10px\]">\(E\)<\/span>\s*<\/th>\s*<\/tr>/,
...
// 5. Row
content = content.replace(
  /<ArrowRight className="w-3 h-3 text-cyan-600 group-hover:translate-x-0.5 transition-transform" \/>\s*<\/button>\s*<\/td>\s*<\/tr>/g,
...
fs.writeFileSync('src/components/TypeView.jsx', content);
console.log('TypeView.jsx patched');
