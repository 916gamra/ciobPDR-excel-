        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-100">
          ...
          <div className="text-xs font-semibold text-slate-500">
            <span className="bg-cyan-50 text-cyan-700 px-2.5 py-1 rounded-lg font-bold border border-cyan-100">
              {filtered.length} type{filtered.length > 1 ? 's' : ''} trouvé
              {filtered.length > 1 ? 's' : ''}
            </span>
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                <label className="text-[11px] font-bold uppercase tracking-wider text-slate-500">
                  ID / Nom du Type (ex: Foret, Vis, Roulement)
                </label>
                <input
                  type="text"
                  ...
                  onChange={(e) =>
                    setForm({
                      ...
                      libelle: form.libelle || e.target.value,
                    })
                  }
